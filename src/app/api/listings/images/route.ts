...
export const POST = async (req: Request): Promise<Response> => {
  ...
  const rate = checkRateLimit(
    `listing-image:${clientKeyFromRequest(req)}`,
    30,
    60_000,
  );
  if (!rate.ok) {
  ...
  if (isCloudinaryConfigured()) {
    try {
      ...
      console.error("[listing-image] Cloudinary upload failed:", err);
      return NextResponse.json(
        { error: "Image upload to storage failed. Try again or use an image URL." },
        { status: 502 },
      );
    }
  }
  ...
  if (process.env.VERCEL === "1") {
    return NextResponse.json(
      {
        error:
          "File upload requires Cloudinary on Vercel. Set CLOUDINARY_CLOUD_NAME, CLOUDINARY_API_KEY, and CLOUDINARY_API_SECRET (or CLOUDINARY_URL), or paste HTTPS image URLs.",
      },
      { status: 400 },
    );
  }
  ...
  const publicPath = `/uploads/listings/${session.sub}/${filename}`;
